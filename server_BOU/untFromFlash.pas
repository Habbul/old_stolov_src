...
begin
  ...
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  // при любой ошибке брать последнюю принадлежащую данной tradepoint строку
  // в таблице sale и записывать в файл tradepoint_ok
  //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  frmFromFlash.pbExport.Position := frmFromFlash.pbExport.Position + pos;
  NameF := FlashButton + ':/' + IntToStr(TradePoint_id) + '_Wait.bou';
  if not FileExists(NameF) then         // ищем на флешке файл-флаг ожидания подтверждения выгрузки
  begin                                 // имя файла состоит из TradePoint_Wait.bou
    result := false;                    // файл не найден, выдаем ошибку!!! 
    MessageDlg('Ошибка при выгрузке продаж!' + #13#10 + 'Не найден файл обмена!', mtError, [mbOk], 0);
    exit;
  end;
  ...
  if sDate = '' then                    // данных нет - выдаем ошибку
  begin
    MessageDlg('Ошибка при выгрузке ведомости!' + #13#10 + 'Файл обмена пуст!', mtError, [mbOk], 0);
    result := false;
    exit;
  end;

  if not FileExists(FlashButton + ':/' + IntToStr(TradePoint_id) + '_sale.bou') then // ищем файл продаж
  begin                                 // файл не найден, выдаем ошибку!!!
    result := false;
    MessageDlg('Ошибка при выгрузке продаж!' + #13#10 + 'Не найден файл данных!', mtError, [mbOk], 0);
    exit;
  end;
  ...
  frmFromFlash.pbExport.Position := frmFromFlash.pbExport.Position + pos;
  frmFromFlash.qImport.Close;           // очищаем временную таблицу tempsale
  ...
  frmInFlash.qRoot.SQL.Clear;           // выгружаем sale.bou во временную таблицу tempsale
...
//    frmFromFlash.Memo1.Lines.Add(DateTimeToStr(now) + ': ' +  frmFromFlash.qTempSale.FieldByName('saletime').AsString);
    case frmFromFlash.qImport.FieldByName('cou').AsInteger of
      0: // такой записи нет в sale - добавляем
      begin
//        s := ;
      ...
      1:// такая запись есть в sale - отбрасываем ее, т.е. ничего не делаем
      begin
...
{  frmFromFlash.pbExport.Position := frmFromFlash.pbExport.Position + pos;
  frmFromFlash.qImport.Close;
  frmFromFlash.qImport.SQL.Clear;       // удаляем из таблицы tempsale записи которые есть в sale...
  frmFromFlash.qImport.SQL.Add('delete from tempsale ');
  ...
  frmFromFlash.pbExport.Position := frmFromFlash.pbExport.Position + pos;
  frmFromFlash.qImport.Close;
  frmFromFlash.qImport.SQL.Clear;       // ...все остальные записи добавляем в таблицу sale
...
{  frmFromFlash.pbExport.Position := frmFromFlash.pbExport.Position + pos;
  frmFromFlash.qImport.Close;
  frmFromFlash.qImport.SQL.Clear;       // очищаем tempsale
  frmFromFlash.qImport.SQL.Add('delete from tempsale');
  frmFromFlash.qImport.Execute(true);   }
  ...
  if FileExists(FlashButton + ':/' + IntToStr(TradePoint_id) + '_Wait.bou') then  // удаляем файл-флаг ожидания
    DeleteFile(FlashButton + ':/' + IntToStr(TradePoint_id) + '_Wait.bou');

  if FileExists(FlashButton + ':/' + IntToStr(TradePoint_id) + '_sale.bou') then  // удаляем sale.bou
...
//           ' ' + IntToStr(wHour) + ':' + IntToStr(wMin) + ':' + IntToStr(wSec);
  ...
  // ...и записываем в файл-флаг подтверждения
  ...
  FileSetAttr(NamFile, faHidden);// устанавливаем аттрибут - скрытый
  CloseFile(TextF);
  frmFromFlash.qImport.Close;
end;
...
begin
  ...
  frmUpdateNew.qTemp.SQL.Add('values(Now(), 1, 13, "Начало выгрузки продаж с флешки от ' + cbPunct.Text + '")');
  ...
  begin
    ...
    begin
      ...
      begin // обнаружили букву флешки
        ...
        TradePoint_id := frmFromFlash.cbPunct.ItemIndex; // определяем точку продаж

        if fncImportSale() then
          pnlStep4.Visible := true
        else // ошибка при выгрузке, создать файл обмена TradePoint_Ok.bou с датой последней загруженной продажи
        begin
          frmFromFlash.qImport.Close;
          frmFromFlash.qImport.SQL.Clear;       // берем последнюю запись в sale принадлежащую данной tradepoint...
          frmFromFlash.qImport.SQL.Add('select max(saletime) as MaxST from sale where tradepoint_id = ' + IntToStr(TradePoint_id));
...
//загружаем все во временную таблицу
Query_temp.Close;
Query_temp.SQL.Clear;
...
//находим максимальную запись в Historyevents_frompoints по данной точке
max_id:=0;
Query_temp.Close;
Query_temp.SQL.Clear;
...
//стираем из temphistory все, что меньше этой записи
Query_temp.Close;
Query_temp.SQL.Clear;
Query_temp.SQL.Add('delete from tempHistory');
...
//записываем все остальное в Historyevents_frompoints
Query_temp.Close;
Query_temp.SQL.Clear;
...
//сохраняем номер последней вставленной записи
...
FileSetAttr(Copy_history_name, faHidden);         // устанавливаем аттрибут у файла - скрытый
CloseFile(Copy_History);
  ...
  frmUpdateNew.qTemp.SQL.Clear;
  frmUpdateNew.qTemp.SQL.Add('insert into historyevents(OperTime, Empl_id, Event_id, memo)');
  frmUpdateNew.qTemp.SQL.Add('values(Now(), 1, 14, "Окончание выгрузки продаж с флешки от ' + cbPunct.Text + '")');
  frmUpdateNew.qTemp.Execute(true);
end;
