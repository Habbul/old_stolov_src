...
//20090325 Lena{
  ...
  begin
    MessageDlg('Введите фамилию!', mtError, [mbOK], 0);
    edFIO.SetFocus;
    exit;
  end;
  if trim(edName.Text) = '' then
  begin
    MessageDlg('Введите имя!', mtError, [mbOK], 0);
    edName.SetFocus;
    exit;
  end;
  if trim(edParentName.Text) = '' then
  begin
    MessageDlg('Введите отчество!', mtError, [mbOK], 0);
    edParentName.SetFocus;
    exit;
  end;
  if trim(edNumBSK.Text) = '' then
  begin
    MessageDlg('Считайте № БСК!', mtError, [mbOK], 0);
    edNumBSK.SetFocus;
    exit;
  end;
  if cbTEmpl.text = '' then
  begin
    MessageDlg('Выберите должность!', mtError, [mbOK], 0);
    cbTEmpl.SetFocus;
    exit;
  end;
