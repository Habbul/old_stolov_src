...
begin
  ...
  true: //режим выборки данных для отчета
    begin
...
function fncUpdStopList() : boolean; // обновление стоплиста
var
  sStopList : string;
...
try
  ...
  if FileExists(sStopList + sTabNamStopList) then  //поиск файла данных стоп-листа
  begin
    ...
    frmSpr.qStopList.SQL.Clear;                    //очищение старого стоп-листа //удаляем записи с флагом 1, то есть те, которые загружены из 1с
    frmSpr.qStopList.SQL.Add('delete from stoplist where flag=1');               //флаг 2 - записи, которые превысили лимит
    frmSpr.qStopList.Execute;

    frmSpr.tStopList.Open;
    frmSpr.tStopList.First;                        //заполнение стоп-листа
    ...
    DeleteFile(sStopList + sTabNamStopList);       //удаление файла данных стоп-листа
    // сделать запись времени последнего обновления стоп-листа в файл настроек
    tIniSett := TIniFile.Create(PutchFolderPrj + '\Setting.ini');
  ...
  begin
    // взять дату из файла настроек
    tIniSett := TIniFile.Create(PutchFolderPrj + '\Setting.ini');
    frmFirst.lDateStopList.Caption := tIniSett.ReadString('OutputData', 'DateStopList', 'не известно');
    tIniSett.Destroy;
    //
  end;
  frmSpr.qTemp.Close;
...
begin
  frmLogo.lAction.Caption := 'Загрузка стоп-листа';
  frmLogo.Update;
  fncUpdStopList();
end;
