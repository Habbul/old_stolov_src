...
begin
  Memo1.Lines.Add(' --- Чтение номера карты --- ');
  DW := 0;
  MifareCardSerialNoGet(@DW);
  Memo1.Lines.Add(' № карты = ' + IntToStr(DW));
end;
...
var
  ...
  i    : integer;
begin
   Memo1.Lines.Add(' --- Чтение данных с карты --- ');
  ...
  case a of
   ...
   frmMain.qSelPers.Open;
    case frmMain.qSelPers.RecordCount of              // по считанному номеру выбираем персональные данные из базы
  ...
  edYear.Text := IntToStr(2000 + Val[6]);    // преобразовываем значение год/месяц с БСК в дату
  ...
  Edit1.Text:=FloatToStr(StrToFloatDef(edLimit.Text,0)-StrToFloatDef(edOst.Text,0));
  if Val[6] = 0 then
...
begin
  Memo1.Lines.Add(' --- Запись данных на карту --- ');
...
begin
  ...
  Memo1.Lines.Add(' --- Открываем сектор № ' + edSect.Text + ' карты № ' + IntToStr(DW) + ' --- ');
...
begin
  ...
  Val[0] := (HexToInt(sSumma[1] + sSumma[2]));  //остаток
  ...
  Val[3] := (HexToInt(sLim[1] + sLim[2]));   // лимит
  Val[4] := (HexToInt(sLim[3] + sLim[4]));   //
  Val[5] := (HexToInt(sLim[5] + sLim[6]));   //

  Val[7] := StrToInt(edMonth.text)*10 + 1; // период - № половины месяца
  if RadioButton2.Checked then
    Val[7] := StrToInt(edMonth.text)*10 + 2; // период - № половины месяца

  Val[6] := StrToInt(frmMain.edYear.text)-2000;        // период - год
...
begin
  ...
  Val[7] := 0*10 + 1; // период - № половины месяца
  if RadioButton2.Checked then
    Val[7] := 0*10 + 2; // период - № половины месяца
...
//  fncWriteInCard(CurrentCardNo, Sec, Bloc, Val, Error);
  ...
  frmMain.edit1.Clear;
  frmMain.RadioButton1.Checked := true;
  ShowMessage('Заблокирован успешно');
end;
