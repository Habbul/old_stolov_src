...
//  DllName : string; // путь к ДЛЛ
  ...
  arrSumOtdel      : array [1..9] of real; // массив сумм по отделам
...
procedure prcAddBSKInStopList(InList, InBSK : boolean);// БСК в стоп-лист
...
procedure prcBSKBadData(sCardNo : string);// ошибка вданных на БСК
var
  t : string;
begin
  mDB.qTemp.Close;
...
begin
  ...
  mDB.qTemp.Close;
  mDB.qTemp.SQL.Clear;                  // удаляем данные по кредиту
  ...
  mDB.qTemp.ExecSQL;
  if InList then                        // БСК в стоп-лист
  begin
    mDB.qTemp.Close;
    mDB.qTemp.SQL.Clear;
