...
type
  TfrmPaswrdDlg = class(TForm)
    ...
    qBackUp: TMySQLQuery;
    sqlRootBackUp: TMySQLServer;
    qCheckDate: TMySQLQuery;
...
//      frmPaswrdDlg.qSelPrava.SQL.Add('where t.typeempl_id is not null and c.nomer = ' + t + ' ');
      frmPaswrdDlg.qSelPrava.SQL.Add('select e.nomerbsk, t.typeempl_id as typem, prava.prava_id, concat(e.family, ' + ''' ''' +  ' , e.name, ' + ''' ''' +  ' , e.parentname) as persname, ');
      frmPaswrdDlg.qSelPrava.SQL.Add('t.post as pos ');
      frmPaswrdDlg.qSelPrava.SQL.Add('from beznal.employee e ');
      frmPaswrdDlg.qSelPrava.SQL.Add('left join beznal.typeemployers t on (t.typeempl_id = e.typeempl_id) ');
      frmPaswrdDlg.qSelPrava.SQL.Add('left join beznal.pravafortypeemployee pr on (pr.typeempl_id = t.typeempl_id) ');
      frmPaswrdDlg.qSelPrava.SQL.Add('left join beznal.prava on (prava.prava_id = pr.prava_id) ');
      frmPaswrdDlg.qSelPrava.SQL.Add('where t.typeempl_id is not null and e.nomerbsk = ' + t + ' and flag = 1');
